...
fuse the up, gate, and down projections into a single Pallas
kernel that keeps those intermediates in VMEM, removing that communication
entirely, and measure it against the reference on one TPU v5e core under
JAX 0.6.2, in bfloat16.
...
The expert count sweep (Figure~\ref{fig:sweeps}a) shows a rise and then a
collapse. The speedup improves from 1.37 at one expert to 1.75 at 32
experts, then falls to 1.03 at 128. The token count is fixed, so the mean
...
size in FLOPs per byte. Once that number is small, both implementations
spend their time streaming weights and their rates converge. The fusion
...
loss at the smallest point is a pipeline that never fills. One expert and
2048 rows give only 8 row tiles, so the prologue and the epilogue are a
...
The model dimension sweep (Figure~\ref{fig:sweeps}d) is the weakest result.
The speedup peaks at 1.59 at $d = 1024$ and falls away on both sides, to
0.75 at $d = 256$ and 0.91 at $d = 2048$. Both ends are failures of the
block-size rule rather than of the fusion. At $d = 256$ the rule yields
...
separate the two. Holding $df$ fixed does: the three points in
...
traffic stay constant. The fused kernel slows as well, since at $d = 256$ its
matmuls are much narrower in the contracted dimension, but by less than half
as much.

\subsection{Routing skew}

Everything so far uses balanced experts, the one routing distribution a real
MoE never produces. Figure~\ref{fig:skew} scales the router logits by a skew
parameter, so larger values concentrate tokens on fewer experts.
...
below a full tile. What matters is the ratio of group size to tile height,
not the imbalance itself.

\section{Conclusion}

The fused kernel beats three \rd{} calls whenever the intermediates it
removes are large next to the traffic that remains, and whenever there is
enough work to keep its pipeline full. Over the shape sweeps it is faster at
all but three of the 24 points, and the largest speedup measured anywhere is
2.12.

The favourable condition is a wide MLP. At equal FLOPs the speedup rises
from 1.59 at $f/d = 4$ to 2.12 at $f/d = 64$ (Table~\ref{tab:ratio}),
because the intermediate volume the reference moves grows with the aspect
ratio while its FLOPs and its weight traffic do not. At a fixed $d = 1024$
the speedup rises with $f$ to 1.78. Every shape I measured with $f > d$ and
a block size matched to it is faster than the reference.

Two conditions limit the result. The first is the number of rows per expert.
Expert weights are streamed by either implementation and are amortised over
one group only, so once the mean group size approaches the tile height both
become weight-bound. At $e = 128$ the mean group is 128 rows and the speedup
is 1.03. Routing skew acts through the same channel, costing almost nothing
at 8 experts and reducing the speedup from 1.49 to 1.39 at 64. The second
condition is total work. At $m = 2048$ with a single expert there are only 8
row tiles, too few to fill the pipeline, and the kernel is 5\% slower than
the reference. The two remaining losses, at $d = 256$ and $d = 2048$, come
from the single block-size rule rather than from the fusion, and per-shape
tuning of $(b_m, b_i, b_j, b_k)$ is the first thing to try there.
